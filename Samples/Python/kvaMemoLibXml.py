...
kvaErrorText = {
    ...
    KvaXmlStatusERR_VALUE_CONSECUTIVE : "The values are not conecutive; usually idx attributes",
...
class kvaError(Exception):
    ...
    def __str__(self):
        #return "[kvaError] %s: %s (%d)\n%s" % (self.kvalib.fn, kvaErrorText[self.kvaERR], self.kvaERR, self.__kvaXmlGetLastError())
        self.__kvaXmlGetLastError()
        return "[kvaError] %s: %s (%d)\n" % (self.kvalib.fn, kvaErrorText[self.kvaERR], self.kvaERR)
...
class kvaMemoLibXml(object):
    ...
    def __init__(self):
        ...
        self.dll.kvaXmlGetLastError.argtypes = [c_char_p, c_uint, POINTER(c_int)]
        #self.dll.kvaXmlGetLastError.errcheck = self._kvaErrorCheck

        self.dll.kvaBufferToXml.argtypes = [c_char_p, c_uint, c_char_p, POINTER(c_uint), POINTER(c_long), c_char_p]
        self.dll.kvaBufferToXml.errcheck = self._kvaErrorCheck

        self.dll.kvaXmlToBuffer.argtypes = [c_char_p, c_uint, c_char_p, POINTER(c_uint), POINTER(c_long)]
        self.dll.kvaXmlToBuffer.errcheck = self._kvaErrorCheck


        # msg = create_string_buffer(1*1024)
        # err = c_int(-1)
        # self.dll.kvaXmlGetLastError(msg, sizeof(msg), byref(err))
        # print "=%d===%s=====" % (err.value, msg.value)
    ...
    def kvaBufferToXml(self, conf_lif):
        ...
        self.dll.kvaBufferToXml(c_char_p(conf_lif), len(conf_lif), xml_buf, byref(xml_size), byref(version), script_path)
    ...
    def kvaXmlToBuffer(self, conf_xml):
        ...
        self.dll.kvaXmlToBuffer(c_char_p(conf_xml), len(conf_xml), lif_buf, byref(lif_size), byref(version))
